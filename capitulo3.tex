...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{bilineal_horiz.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.6\textwidth]{/mapa_cd/suavizado.pdf}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=.5\textwidth]{peaks3d.png}
...
\begin{figure}[h!]
    \centering
    \includegraphics[width=\textwidth]{comparativa_interpolacion.pdf}
...
$C_{mult}\in[1.2, 2]$\;\\ }
  ...
  \eIf{$u_{min}> aux = (C_{mult}\cdot u_{medio} - u_{max}) \mathbin{/} (C_{mult}-1)$
    ...
    $\Delta u = u_{max}-u_{medio}$\;
    $a = (C_{mult} - 1) \cdot u_{medio} / \Delta u$\;
    $b = u_{medio} \cdot (u_{max} - C_{mult} \cdot u_{medio}) \Delta u $\;
  }{
    \eIf{$\Delta \neq 0$}{
      $a = u_{medio} \mathbin{/} \Delta u$\;
      $b = -u_{min} \cdot u_{medio} \mathbin{/} \Delta u$ \;
    }{
      $a=1$\;
      $b=0$\;
    }
