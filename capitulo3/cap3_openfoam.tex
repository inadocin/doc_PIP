...
% \subsubsection{Esquemas de discretización}
% %
% La configuración concreta de los esquemas de discretización utilizados se
% definen en el archivo de configuración denominado ``fvSchemes''.

% Para el solver de flujo compresible ``rhoPimpleFoam'' se utilizó:

% \begin{itemize}
%         \item ``ddtSchemes'': Euler
%         \item ``gradSchemes'': cellLimited Gauss linear 1
%         \item ``divSchemes'':
%         \begin{itemize}
%                 \item $\nabla \cdot (\phi,U)$: Gauss linearUpwind $\nabla U$
%                 \item $\nabla \cdot (\phi,e)$: Gauss limitedLinear 1
%                 \item $\nabla \cdot (\phi,h)$: Gauss linearUpwind $\nabla h$
%                 \item $\nabla \cdot (\phi,p)$: Gauss limitedLinear 1;
%                 \item $\nabla \cdot (\phi,K)$: Gauss linear;
%                 \item $\nabla \cdot (\phi,p)$: Gauss limitedLinear 1;
%                 \item $\nabla \cdot (\phi,k)$: Gauss upwind;
%                 \item $\nabla \cdot (\phi,\epsilon)$: Gauss upwind;
%                 \item $\nabla \cdot (((\rho*\nu_{Eff}) dev2((\nabla U)^{T})))$ Gauss linear;
%         \end{itemize}
%         \item ``laplacianSchemes'': Gauss linear limited corrected 0.33
%         \item ``interpolationSchemes'': linear
%         \item ``snGradSchemes'': limited corrected 0.33;
% \end{itemize}

% Para el solver ``pimpleFoam'':
...
%                     $\nabla \cdot (\phi,k)$: Gauss upwind
%                     $\nabla \cdot (\phi,\epsilon)$: Gauss upwind
%                     $\nabla \cdot (\phi,R)$: Gauss limitedLinear
%                     $\nabla \cdot R$: Gauss linear
%                     $\nabla \cdot (\nu_{EFf}*dev2((\nabla U)^{T}))$: Gauss linear
%         \end{itemize}
%         \item ``laplacianSchemes'': Gauss linear uncorrected
%         \item ``interpolationSchemes'': linear
%         \item ``snGradSchemes'': uncorrected
% \end{itemize}
